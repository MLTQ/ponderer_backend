...
- **Interacts with**: `config::AgentConfig`, `database::AgentDatabase`, `memory` module (compiled for backend abstractions), `agent::Agent`, `skills::Skill`, `skills::graphchan::GraphchanSkill`, `tools::{shell, files, http, memory, skill_bridge, comfy, vision}`, `ui::app::AgentApp`
