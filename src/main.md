...
- **Interacts with**: `config::AgentConfig`, `database::AgentDatabase`, `memory` module (compiled for backend abstractions), `agent::Agent`, `skills::Skill`, `skills::graphchan::GraphchanSkill`, `tools::{shell, files, comfy, vision}`, `ui::app::AgentApp`
...
- Tool registry includes shell/files plus media-capable tools (`generate_comfy_media`, `post_to_graphchan`, `evaluate_local_image`, `publish_media_to_chat`, `capture_screen`) for private-chat agentic runs.
