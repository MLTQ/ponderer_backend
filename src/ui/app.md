...
### `AgentApp`
- **Does**: Central application state holding agent reference, event log, UI panels, avatar set, database handle, conversation list/selection, scoped chat history, and live streaming preview state, plus activity panel visibility
...
- **Does**: Main render loop. Loads avatars on first frame, polls `event_rx` for `AgentEvent`s, maps `ChatStreaming` events into a live preview bubble, refreshes chat on operator-related actions, renders header with sprite, toolbar buttons, primary private chat, secondary activity side panel, multiline chat input, and all modal panels (settings, character, comfy workflow). Persists config and hot-reloads the agent on save.
...
| `agent::AgentEvent` | Streaming updates include `conversation_id`, partial/full `content`, and `done` state | Changing streaming event payload shape breaks preview rendering |
